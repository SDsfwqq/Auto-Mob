...
local ATTACKS_PER_TARGET = 10
local TELEPORT_DISTANCE_THRESHOLD = 12
local TARGET_RECHECK_DELAY = 1.25
local ATTACK_INTERVAL = 0.15
local priorityMobs = { "Walrus", "Ankylosaurus", "Argentavis", "Stegosaurus" }
local mobNames = { "Dimorph", "Ptero", "Stego" }
...
-- 🎮 FPS Booster: Hardcore กากสุด
task.spawn(function()
	local ws = workspace
	for _, obj in ipairs(ws:GetDescendants()) do
		if obj:IsA("BasePart") then
			obj.Material = Enum.Material.SmoothPlastic -- ลบพื้นผิว
			obj.Reflectance = 0
	...
	print("⚠️ FPS Booster: โหมดกากสุด เปิดใช้งาน")
end)


print("✅ Loaded! พร้อมฟาร์มแบบเทพ 🚀")
